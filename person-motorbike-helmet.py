    ...
    try:
        ...
        for i in np.arange(0, detections.shape[2]):
            ...
            if confidence > 0.5:
                ...
                if idx == 15:     # if the idx is 15, then the detected element is person
                    box = detections[0, 0, i, 3:7] * np.array([w, h, w, h])
                    (startX, startY, endX, endY) = box.astype("int")
                    # roi = box[startX:endX, startY:endY/4] 
                    # person_roi.append(roi)
                    persons.append((startX, startY, endX, endY))     # storing the coordinates of the detected object

                if idx == 14:    # if the idx is 14, then the deteted element is motorbike
                    box = detections[0, 0, i, 3:7] * np.array([w, h, w, h])
                    (startX, startY, endX, endY) = box.astype("int")
                    motorbi.append((startX, startY, endX, endY))     # storing the coordinates of the detected object
        ...
        for i in motorbi:     # for each tuple of detected motorbike (containing coodinates)....
            mi = float("Inf")
            for j in range(len(persons)):    # and for each tuple of detected person (containing coodinates)....
